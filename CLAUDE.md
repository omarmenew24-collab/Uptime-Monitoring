...
- Don't over-engineer — build what's needed now, not for hypothetical future requirements. **Exception:** the infrastructure in `system-design-roadmap.md` (queue, cache, events, rollups) is in-scope product behavior, each justified by a forcing requirement — it is not "hypothetical future." Build it one phase at a time, in order (1→2→3→4→5), gated by "previous phase done." The anti-cargo-cult refusals (sharding, Kafka, CQRS, etc.) still apply — only refuse what no requirement forces. Refer to the roadmap before treating any listed item as over-engineering.
